...
By default, calling `eso.retrieve_data` submits a new archive request through 
the web form to stage and download the requested `datasets`.  If you would
like to download datasets from an existing request, either submitted through
the functions here or externally, call `retrieve_data` with the `request_id`
option:
...
reverse checking that the specified `datasets` are provided in `request_id`.
